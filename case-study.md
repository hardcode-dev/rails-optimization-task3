...
- [Case-study оптимизации 1](#case-study-оптимизации-1)
  ...
  - [Знакомство с инструментами и анализ](#знакомство-с-инструментами-и-анализ)
    ...
    - [Оптимизация 4: put\_copy\_data](#оптимизация-4-put_copy_data)
    - [Итоги](#итоги)
...
### Оптимизация 4: put_copy_data

```ruby
ActiveRecord::Base.connection.raw_connection.put_copy_data(
```
Скорость загрузки large.json уменьшилась до 30 секунд
Но столкнулся с проблемой, добавил следующий индекс
```ruby
add_index :trips, [:from_id, :to_id, :bus_id, :start_time], unique: true, algorithm: :concurrently
```
Оказалось, что есть некорректные данные в файле large.json, я вот не знаю, что с ними делать, с точки зрения задачи- надо просто раскинуть индекс без проверки на уникальность, с точки зрения домена- ну че, автобус за 188 минут высаживает тех, кто больше заплатил, а потом наматывает штрафные круги над столицей?
```json
{"from":"Тула","to":"Москва","start_time":"16:28","duration_minutes":188,"price_cents":420,"bus":{"number":"742","model":"Спринтер","services":["Ремни безопасности","Телевизор общий"]}}
{"from":"Тула","to":"Москва","start_time":"16:28","duration_minutes":60,"price_cents":96685,"bus":{"number":"742","model":"Спринтер","services":["Ремни безопасности","Телевизор общий"]}}
```
...
Медленной работы приложения эмпирическим путем я не наблюдаю (P.S. потому что накатил маленький файл с данными)
...
Указанный запрос говорит мне, что самое большое количество маршрутов для комбинации городов составляет ~~7 штук~~ (P.S. накатил не тот файл с данными.)
...
И ловим проблемку. Оказывается в файле импорта города мы создаем дубликаты по :name.
В нормальной ситуации пришлось бы делать сложную миграцию, чтобы исправлять текущие данные, но с учетом того, что задание учебное- я просто снесу все записи, создам индекс и перенакачу города
...
1) Такое ощущение, что выгрузка логов для pg_badger сильно замедляет работу, поэтому вопрос, а как в реальном проде обстоят дела с этим? Все логи собираются нонстопом и в нужный момент по нужной выборке запускается pg_badger? Влияет же на производительность, не?
...
3) В pg_hero можно как-то отключить мониторинг мусорных запросов?

4) pg_hero какой-то мутный (то показывает, то не показывает, то вмного служебных запросов), pg_badger- неудобный для дебага, консоль рельсы не работает для rake задач, new-relic для такого проекта как из пушки по воробьям, такая же проблема с prometheus+grafana, ruby-prof теряется в стеке рельсовых вызовов, а хочется что-то типа rails_profiler, только для отладки rake-скриптов в консоли. Есть какой-то компромисс между всем этим?

## Action Items

*) не стоит бросаться писать индексы, если не знаешь предметной области
*) стоило поставить NewRelic, но я подсмотрел ДЗ4, поковыряюсь с ним там
*) "лучше сначала заполнить данными, потом накатить индекс"- такое утверждение встретил в материалах курса, а вот в ДЗ был кейс, что я бы поспорил, на самом деле
*) опасность операций над базой (индексы накатывал с disable_ddl_transaction, в итоге миграция показывала ошибку, но частично индекс накатывала и сама миграция отмечалась пройденной)
*) rack-mini-profiler - не оценил, но, возможно, будет возможность в ДЗ4
*) проигнорировал
  - strong_migrations gem
  - не использовал explain analyze и тулзы с визуализацией (потому что запросы простые и потому что хочется взять готовый запрос, а не генерировать вручную/подставлять вместо значков $1 какие-то данные)
  - не пришлось встретиться с другими индексами (gist, gin) и поработать с jsonb
