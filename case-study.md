...
## Setup
До начала всех оптимизации я установил progressbar в rake таск импорта и замерил время выполнения:
Получилось
- small.json ~7 секунд
...
Фидбек луп стандартный, требует около 10 секунд для получения первых результатов:
- внесение изменений в код
– запуск тестов
- анализ данных профилировщиков
- к шагу 1
...
- Метрика realtime улучшилась 7.30 -> 7.0, из Explain ушел Seq Scan и сменился на Index Scan using index_buses_services_on_bus_id on buses_services 
...
# Оптимизация 3
- По отчету pghero в топе запрос на добавление
```
...
- Результатов эта оптимизация не дала. Время выполнения скрипта не изменилось. Explain запроса по-прежнему показывал Seq Scan...Почему так – установить не удалось.
...
- В топе pghero находился  запрос
```
SELECT  $4 AS one FROM "buses" WHERE "buses"."number" = $1 AND "buses"."id" != $2 LIMIT $3
```
...
- первым делом я изменил индекс по полю number на уникальный. Сходу подумал, что может быть связано с этим. Но не помогло. Затем я посмотрел в логи, в каком месте вызывается этот запрос и увидел, что он вызывается на строчке кода 
```
...
На целевом объеме large время стало уже приемлемым, приближающимся к минуте. 
При прогоне на объеме large, накапливании trips и вставкой в конце время работы скрипта, время выполнения ~80 секунд. Я добавил вставку при накапливании меньшего количества записей. При вставке по 10к записей время работы скрипта стало ~60 секунд
...
- в результате SELECT ушел из топа запросов. Скрипт прогнался за 24 секунды на объеме large. Таким образом, получилось уложиться в бюджет.
...
## Оптимизация 1 Bus N + 1
- Сразу после установки bullet он выдавал предупреждение N+1 и давал рекомендацию добавить в запрос includes(:bus)
