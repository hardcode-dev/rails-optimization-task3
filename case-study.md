...
Удалось сократить время выполнения импорта данных из `large.json` файла, так что бы уложиться в заданный буджет `60 sec`.
На данном этапе оптимизации импорт `large.json` файла выполняется `~ 45 sec`. 
