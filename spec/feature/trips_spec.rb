...
RSpec.describe 'Trips', type: :feature do
  before(:all) { ImportJSONService.perform('fixtures/example.json') }
  after(:all) { [City, Bus, Service, Trip, BusesService].each(&:delete_all) }
  let(:text_trip_1) {
    [
      'Отправление: 17:30',
      'Прибытие: 18:07',
  ...
  it 'shows example trips index' do
    visit trips_url('Самара', 'Москва')
